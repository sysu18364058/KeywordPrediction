...
@dataclass
class PredictDatacollator(DataCollatorForLanguageModeling):
    ...
    def __call__(self, 
                     examples: list):
        '''
        Returns dict{"input_ids": [batch, tokens_id],
                     "label": [batch, label_tokens_id]}
        '''
        all_sens = []
        labels = []
        for ex in examples:
...
@dataclass
class WWMDataCollator(DataCollatorForLanguageModeling):
